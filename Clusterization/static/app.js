...
canvas.addEventListener(
  "click",
  function (
    mouseEvent // ставит точки на экране
  ) {
    if (flagPoints === true) {
...
function Clustering(numberClust) {
  ...
  while (
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      let distancesColors = centersColors.map((center) =>
        getDistance(points[i], center)
      );
      let distancesBlack = centersBlack.map((center) =>
        getDistance(points[i], center)
      );
      let distancesLines = centersLines.map((center) =>
        getDistance(points[i], center)
      );

      let minIndexColors = distancesColors.indexOf(
        Math.min(...distancesColors)
      );
...
function getDistance(point1, point2) {
  return Math.sqrt(
    Math.pow(point1.pointX - point2.pointX, 2) +
      Math.pow(point1.pointY - point2.pointY, 2)
  );
}
...
function drawHierarchy() {
  for (let i = 0; i < clustersHierarchy.length; i++) {
    for (let j = 0; j < clustersHierarchy[i].length; j++) {
      ...
      context.fillStyle = "black";
      context.arc(
        clustersHierarchy[i][j].pointX,
        clustersHierarchy[i][j].pointY,
        10,
        getRadians(blackColoring[i][0]),
        getRadians(blackColoring[i][1])
      );
      context.fill();
      context.stroke();
    }
